...
interface ISigmoidBank{
    ...
    function getBondExchangeRateSGMtoSASH(uint256 amount_SGM_out) view external returns (uint256);
...
contract SigmoidBank is ISigmoidBank,swap{
    ...
    function bigestLP() view public returns (address) {
        ...
        return(Bigest_LP_address);
    }
    
    //returns pair address
    function bigestUSDToken() view public returns (address) {
        address Bigest_USD_address;
        address current_LP_address;
        uint256 Bigest_LP_size;
        uint256 current_LP_size;
        for (uint i=0; i<USD_token_list.length; i++){
            ...
            if (Bigest_LP_size < current_LP_size){
                Bigest_LP_size = current_LP_size;
                Bigest_USD_address = USD_token_list[i];
            }
        }
        return(Bigest_USD_address);
    }
    ...
    function getBondExchangeRateSGMtoSASH(uint256 amount_SGM_out) view public override returns (uint256){
    ...
    // function pledgeETHForSASHBond(address _to, address[] memory path) public payable returns (uint[] memory amounts){
  
    //     require(path[0] == WETH, 'INVALID_PATH');
    //     path[1] = bigestUSDToken();
    //     require(path[2] == SASH_contract, 'INVALID_PATH');
    //     require(path.length == 3, 'INVALID_PATH');
        
    //     uint256 amount_SASH_out = IUniswapV2Router01(SwapRouterAddress).getAmountsOut( msg.value, path)[1] ;

        
    //     address pair_addrss=IUniswapV2Factory(SwapFactoryAddress).getPair(WETH, SASH_contract);
        
    //     require(ISigmoidTokens(SASH_contract).bankTransfer(bigestLP(), pair_addrss, amount_SASH_out));
    //     require(IERC20(WETH).transferFrom( msg.sender, pair_addrss, msg.value),'Not enough WETH for the deposit.');
    //     IUniswapV2Pair(bigestLP()).sync;
        

    //     IUniswapV2Pair(pair_addrss).mint(address(this));
    //     IUniswapV2Pair(pair_addrss).sync;
                
    //     IERC659(bond_contract).issueBond(_to, 4, msg.value);
    //     IERC659(bond_contract).issueBond(_to, 5, amount_SASH_out);
    //     return(IUniswapV2Router01(SwapRouterAddress).getAmountsOut( msg.value, path));
    ...
    function buyVoteBondWithSGM(address _from, address _to, uint256 amount_SGM_in) public override returns (bool){
        ...
        uint256 amount_bond_out = getBondExchangeRateSGMtoSASH(amount_SGM_in);
        address pair_addrss=IUniswapV2Factory(SwapFactoryAddress).getPair(SGM_contract,SASH_contract);
        ...
        require(ISigmoidTokens(SASH_contract).bankTransfer(bigestLP(), pair_addrss, amount_bond_out),'Not enough SGM for the deposit.');
        IUniswapV2Pair(bigestLP()).sync;

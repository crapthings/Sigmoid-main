...
interface ISigmoidGovernance{
    ...
    function migratorBond(uint256 poposal_class, uint256 proposal_nonce, address _from, address _to, uint256[] calldata class, uint256[] calldata nonce, uint256[] calldata _amount) external returns(bool);
...
contract SigmaGovernance is ISigmoidGovernance{
    ...
    function migratorBond(uint256 poposal_class, uint256 proposal_nonce, address _from, address _to, uint256[] calldata class, uint256[] calldata nonce, uint256[] calldata _amount) public override returns(bool){
        require(poposal_class <= 2);
        require(checkProposal( poposal_class,  proposal_nonce) == true);
        require(_proposalAddress[poposal_class][_proposalNonce[poposal_class]] == msg.sender);
        require(IERC659(bond_contract).transferBond(_from, _to, class, nonce, _amount));
